...
app.post('/submit', (req, res) => {
  try {
    ...
    if (!name || typeof score !== 'number') return res.status(400).json({ error: 'Bad request' });
    
    // Update leaderboards
    updateLeaderboards(name, score, stats);
    lbBroadcast({ type: 'leaderboards', payload: {
      points: top.points, kills: top.kills, asteroids: top.asteroids, belt: top.belt, survival: top.survival
    }});
...
app.post('/blockchain/submit', async (req, res) => {
  ...
  try {
    ...
    if (extensionResponse && extensionResponse.approved) {
      ...
      console.log('✅ Extension approval received, submitting to blockchain...');
      
      // Prepare data for blockchain storage
      const dataBytes = new TextEncoder().encode(JSON.stringify({
        game: 'Orbit Runner',
        version: '1.0.0',
        playerAddress: playerAddress,
        timestamp: gameData.ts || Date.now(),
...
server.on('upgrade', (req, socket, head) => {
  try {
    ...
    if (url.pathname === '/mp') {
      mpWss.handleUpgrade(req, socket, head, (ws) => mpWss.emit('connection', ws, req));
    } else {
      lbWss.handleUpgrade(req, socket, head, (ws) => lbWss.emit('connection', ws, req));
    }
  } catch (e) { 
    socket.destroy(); 
  }
